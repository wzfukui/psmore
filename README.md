...
- `o` 将当前诊断上下文导出为带版本号的 JSON 报告，包括完整进程清单、资源聚合和最近事件；已经打开的端口扫描、深度检查及已捕获基线会一并写入，但导出本身不会触发额外扫描
...
- 诊断报告先写临时文件再原子改名，并在 Unix 平台使用 `0600` 权限。报告可能包含完整命令行、文件路径、用户名、主机名和 socket 端点，分享前应先检查敏感信息。
