...
- TUI 内置中文和英文：首次按 macOS/Linux 系统语言选择，任意界面（文本输入除外）按 `L` 手工切换并持久保存
- 内置深色/浅色/高对比三套配色主题（`--theme`、`PSMORE_THEME` 或命令面板"切换主题"，选择持久保存），并支持纯 ASCII 字符集回退（`--glyphs ascii`、`PSMORE_GLYPHS`，在 `dumb`/`linux` 终端或非 UTF-8 语言环境下自动启用）
...
- 顶部状态栏显示主机名、负载趋势、全局 CPU/内存/Swap 和告警数量；宽终端下进程树带每进程 `CPU%`/`MEM` 列
...
- `p` 打开进程操作中心（TERM/KILL/STOP/CONT）；选择操作后必须另按 `y` 确认，执行前会重新校验 PID 与启动时间，避免把信号发给复用该 PID 的新进程
- PID 0、PID 1 和 `psmore` 自身受强制保护；每次已发送、被安全策略拒绝或被操作系统拒绝的结果都会进入活动审计
- `o` 将当前诊断上下文导出为带版本号的 JSON 报告，包括完整进程清单、资源聚合、当前查询及命中数、关注事项、最近事件和进程操作审计；已经打开的网络扫描、深度检查、内存归因、进程档案、服务上下文、原生日志、可执行映像证据及已捕获基线会一并写入，但导出本身不会触发额外扫描
...
- `psmore explain PID` 是面向事故现场的一键进程档案：并行拼合深检、服务归属、可执行映像和原生日志，再按严重程度列出可追溯到原始证据的关注事项；TUI 中按 `D` 打开同一份档案
...
psmore 的交互界面内置中文和英文。第一次启动时自动读取操作系统语言：macOS 使用全局 `AppleLanguages`/`AppleLocale`，Linux 和其他 Unix 环境使用标准 locale；中文环境启用中文，其他语言环境回退英文。之后在任意界面（包括打开的面板，文本输入中除外）按 `L` 即可立即切换语言。手工选择会写入同一份私有偏好文件，并在后续启动时优先于系统语言。临时指定首次语言可设置 `PSMORE_LANG=zh_CN` 或 `PSMORE_LANG=en_US`。命令行表格、JSON schema、查询字段和脚本输出继续使用稳定英文，避免破坏自动化。

第一次进入交互界面时，psmore 会展示三页现场手册，将主要能力按“理解进程树 → 从症状找到证据 → 安全操作与分享”组织起来。使用 `←`/`→`、`↑`/`↓` 或 `Tab` 翻页，`Enter`/`Esc` 开始工作；`q` 关闭当前手册，`Ctrl-C` 始终可以直接退出。

完成首次手册后，每次启动会在右下角展示一条高级 tip，覆盖子树查询、热点、基线、网络、趋势、内存归因、进程档案、原生日志、映像验证和安全操作；14 条完成后从第一条继续轮播，直到用户主动关闭。Tip 不会抢走工作键：除 `Enter`、`Esc`、`q`、`?`、`T`、`D`、`L` 外，按下任意键都会关闭 tip 并继续执行该键原本的操作，例如 `Space` 仍会立即暂停、`/` 仍会开始查询。
...
发布归档记录源码 commit、dirty 状态、目标架构、Rust 版本和固定时间源，并生成 SHA-256 校验文件。SHA-256 用于确认下载完整性，不代替发布者身份签名；应从可信的发布页面同时取得归档和校验文件。详细发布流程见 [`docs/RELEASING.zh-CN.md`](docs/RELEASING.zh-CN.md)。
...
现场看到一个可疑 PID 时，通常还要依次检查运行上下文、服务管理器、磁盘上的程序是否已替换，以及最近日志。`explain` 把这四段只读证据并行采集成一份档案，并先给出按 `critical / warning / notice` 排序的线索，再附上每个原始报告：
...
四个采集器共享初始 PID 和启动时间作为档案身份，完成后还会再次刷新目标；任何分段 PID/启动时间不一致都会被拒绝归因，整个采集期间确认 PID 复用则整份报告失败。某项因权限、平台能力或竞争条件只得到部分证据时，section 会标记 `partial`，不会把“未看到”解释成“不存在”。
...
TUI 中选中进程按 `D` 打开同一份摘要；`s`/`p`/`w` 调整日志边界，`g` 开关日志，`h` 开关哈希，`i`/`m`/`v`/`l` 跳到四类原始证据。采集始终在后台进行，`o` 会把已完成的档案嵌入诊断报告但不会为了导出额外触发采集。CLI 的 `--output` 强制写 JSON，使用 mode `0600` 的私有原子文件并默认拒绝覆盖。指定 `--no-logs` 后不能再同时指定 `--scope`、`--since`、`--priority` 或 `--limit`，避免接受实际不会生效的参数。档案可能包含命令参数、路径、用户、文件/网络资源、服务配置、哈希、签名和业务日志；对外分享前应使用 `--redact`，并再次人工检查。
...
| `/` | 进入搜索输入；输入期间不改变进程树，`j/k` 也是普通字符；`↑`/`↓` 回现已应用的查询历史（最近 20 条，持久保存），`Tab` 补全查询字段（如 `name:`、`cpu>`）；按 `Enter` 才应用搜索并回到选中模式，此时 `p` 可操作选中进程；结果状态下按 `Esc` 清除搜索 |
| `:` | 打开命令面板；输入时按中英文名称和关键词模糊匹配全部功能，`↑`/`↓` 选择，`Enter` 执行（与对应按键行为完全一致），`Esc` 关闭；文本输入中 `:` 仍是普通字符 |
| `*` | 为选中进程加/取消星标；星标绑定进程实例（PID + 启动时间），PID 复用不会继承，仅保留在会话内 |
| `'` | 跳到下一个星标进程（循环）；没有星标或星标不可见时给出提示 |
| `f` | 聚焦当前节点的父链和子树 |
...
| `x` | 清除当前基线；在差异面板内同样可用 |
| `n` | 打开全局网络面板；`v` 切换监听/全部连接，`/` 文本过滤，`p` 输入端口号精确定位（本地或对端端口），`x` 清除两类过滤，`r` 重新扫描，`Enter` 跳到所有者进程，`Esc` 或 `n` 关闭 |
| `D` | 并行建立选中进程的档案，汇总深检、服务归属、运行映像与日志并优先展示异常线索；`i`/`M`/`m`/`v`/`l` 跳到相关证据，`g` 切换日志采集，`h` 切换哈希，`Esc` 或 `D` 关闭 |
| `M` | 在后台归因选中进程的内存，展示 RSS/PSS/footprint、匿名/文件/共享页、Swap、区域和映射；`Enter`/`r` 刷新，`D` 跳到进程档案，`i`/`m`/`v`/`l` 跳到相关证据，`Esc` 或 `M` 关闭 |
...
| `p` | 打开选中进程的操作中心；选择 TERM/KILL/STOP/CONT 后，另按 `y` 才发送信号，`Esc` 取消；这是唯一的信号入口（旧的 `k` 结束弹窗已移除，`k` 改为上移） |
| `o` | 将当前诊断上下文导出到工作目录中的 `psmore-report-*.json`；任意面板内均可使用 |
| `L` | 在任意界面（包括打开的面板和弹窗）切换中文/英文，选择会持久保存；文本输入中 `L` 仍是普通字符 |
...
| `q` | 关闭当前面板、工作区或弹窗；仅在裸进程树上退出 psmore；`Ctrl-C` 在任意状态立即退出 |

鼠标：单击进程树行选中该行，再次单击已选中行打开深度检查；深检面板的卡片标签可点击切换；滚轮等同于 `↑`/`↓`（移动选择或滚动当前面板）；其余位置的点击被忽略。
...
- 诊断报告 schema v8 先写临时文件再原子改名，并在 Unix 平台使用 `0600` 权限。报告可嵌入已采集的进程档案和内存归因，并可能包含完整命令行、文件路径、用户名、主机名、线程名、socket 端点、systemd/launchd 服务上下文、原生日志、内存布局、可执行映像哈希与签名和人工操作审计，分享前应先检查敏感信息。
